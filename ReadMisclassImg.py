...
def ReadMisFile(path):
    ...
    pred_lable = [row[-1] for row in arr]
...
def copy_img():
    ...
    local_img_dir = r'dataset'  # 训练集文件路径，local_img_dir + 原始标签 + 文件名 = 错误分类文件路径
    CSV_file_path = r'filename.csv'  # CSV文件路径
    save_dir = r'Misclassified'  # 错误影像另存的文件夹路径
    ...
    for i in range(len(file_name_list)):
        ...
        mkdir(img_save_path)
        shutil.copy(img_file_path,img_save_path+ '/' + file_name_list[i])
